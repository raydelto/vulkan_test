...
int VulkanRenderer::init(GLFWwindow * newWindow)
{
	...
	try {
		...
		uboViewProjection.projection = glm::perspective(glm::radians(90.0f), (float)swapChainExtent.width / (float)swapChainExtent.height, 0.1f, 200.0f);
...
void VulkanRenderer::getPhysicalDevice()
{
    ...
    std::cout << "Number of GPUs: " << deviceList.size() << std::endl;
	...
	for (const VkPhysicalDevice &device : deviceList)
	{
		if (checkDeviceSuitable(device))
		{
            ...
            std::cout << "DeviceSuitable: " << device << std::endl;
			break;
		}
		else
		{
			std::cout << "Device NOT Suitable: " << device << std::endl;
		}
	}
